...
public interface UserJPAMapper extends EntityMapper<UserJPAEntity, User>{
   ...
   default User toDomain(UserJPAEntity dto)
   {
       ...
       List<Product> products = dto.getWatchList().stream().map(p -> {
           ...
           product.setSustainabilityCriteria(new SustainabilityCriteria(p.getSustainabilityCriteria().getCarbonFootprint(), p.getSustainabilityCriteria().getEnergyEfficiency(), p.getSustainabilityCriteria().getResourceUsage(), p.getSustainabilityCriteria().getWasteManagement()));
           return product;
       }).toList();
